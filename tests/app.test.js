...
// fetch mockado — captura a última chamada e devolve o que o teste configurar em __fetchMock.
let fetchCalls = [];
let fetchMock = async () => new Response('{}', { status: 200 });
...
const sandbox = {
  ...
  fetch: mockFetch,
  Response, // built-in do Node, exposto explicitamente pro sandbox
};
...
;globalThis.__exports = {
  ...
  optionName, buildSolicitanteMaps, solicitanteDisplayName, migrateLegacyUserIdx,
  filtrarAnexosValidos, waNumberForTask, getCustomField, slaProgressInfo,
  authRequest, authErrorMessage,
...
await test('null sem due_date/date_created válidos', () => {
  assert.strictEqual(A.slaProgressInfo({ status: { status: 'aberto' } }), null);
});
await test('pct fica sempre entre 0 e 100', () => {
...
await test('STATUS_MAP tem exatamente os 4 status esperados', () => {
...
await test('monta o mapa nome -> orderindex da ClickUp corretamente', () => {
...
await test('monta o mapa reverso orderindex -> nome corretamente', () => {
...
await test('sortedOptions fica em ordem alfabética, independente da ordem de entrada', () => {
...
await test('sortedOptions preserva o orderindex original de cada nome (não reindexa)', () => {
  const { sortedOptions } = A.buildSolicitanteMaps(FAKE_CU_OPTIONS);
...
await test('tem os 42 nomes da versão antiga (0.2.4), sem duplicar orderindex', () => {
...
await test('trava os índices do bug reportado em 2026-07-23', () => {
...
await test('converte user_idx antigo pra user_name e remove a chave antiga', () => {
...
await test('não faz nada se já existe user_name', () => {
  A.store.set('user_name', 'Alguém');
  A.store.set('user_idx', '25');
  A.migrateLegacyUserIdx();
  assert.strictEqual(A.store.get('user_name'), 'Alguém');
});
await test('não faz nada se nunca configurou (sem user_idx nem user_name)', () => {
  A.migrateLegacyUserIdx();
  assert.strictEqual(A.store.get('user_name'), null);
});

console.log('solicitanteDisplayName (sem mapa carregado — app recém-aberto)');
await test('sem mapa carregado retorna placeholder', () => {
  assert.strictEqual(A.solicitanteDisplayName(26), '—');
});
await test('com null/undefined retorna placeholder', () => {
  assert.strictEqual(A.solicitanteDisplayName(null), '—');
  assert.strictEqual(A.solicitanteDisplayName(undefined), '—');
});

console.log('authRequest / authErrorMessage (login com senha)');
await test('authRequest devolve os dados quando o Worker responde 200', async () => {
  fetchMock = async () => new Response(JSON.stringify({ token: 'abc', name: 'Michael Vasconcelos' }), { status: 200 });
  const data = await A.authRequest('/login', { name: 'Michael Vasconcelos', password: 'x' });
  assert.strictEqual(data.token, 'abc');
});
await test('authRequest manda X-App-Secret e o corpo certo', async () => {
  fetchMock = async () => new Response('{}', { status: 200 });
  await A.authRequest('/login', { name: 'Fulano', password: 'senha' });
  assert.strictEqual(fetchCalls.length, 1);
  assert.ok(fetchCalls[0].url.endsWith('/auth/login'));
  assert.strictEqual(fetchCalls[0].opts.headers['X-App-Secret'], 'isv-chamados-2k26-9fQ3vM7xZp');
  assert.deepStrictEqual(JSON.parse(fetchCalls[0].opts.body), { name: 'Fulano', password: 'senha' });
});
await test('authRequest lança erro com .status quando o Worker responde erro', async () => {
  fetchMock = async () => new Response(JSON.stringify({ error: 'Senha incorreta' }), { status: 401 });
  await assert.rejects(
    () => A.authRequest('/login', { name: 'x', password: 'errada' }),
    err => err.status === 401 && err.message === 'Senha incorreta'
  );
});
await test('authErrorMessage repassa mensagem de erro que já tem status (vem do Worker)', () => {
  const err = new Error('Sem senha cadastrada pra esse nome ainda');
  err.status = 404;
  assert.strictEqual(A.authErrorMessage(err), 'Sem senha cadastrada pra esse nome ainda');
});
await test('authErrorMessage cai no tradutor genérico quando não tem status (erro de rede)', () => {
  const err = new Error('Failed to fetch');
  assert.strictEqual(A.authErrorMessage(err), 'Não foi possível conectar. Verifique sua internet e tente novamente.');
});

...
- **Deixou de ser só leitura em 2026-08-07** — decisão do gestor: a TI (Everson/Henrique) para de abrir a ClickUp pra trabalhar e passa a aceitar chamado, mudar status, escrever solução e atribuir operador direto pelo painel (ver `POST /admin/tasks/:id` acima). A ClickUp **continua guardando o dado** (nenhuma migração, nenhuma automação de SLA reescrita) — só deixou de ser a *interface* de trabalho. Cada linha da tabela tem um botão "Gerenciar" que abre um modal (status/operador/solução) pré-preenchido com o estado atual do chamado.

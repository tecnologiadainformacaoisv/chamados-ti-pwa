...
│   └── push-worker.test.js ← testes do Worker (auth, isolamento entre solicitantes)
└── assets/
...
- **O que funciona hoje:**
  ...
  - **Backend do painel de admin, Fase 1** (`GET /admin/tasks`, `GET /admin/metrics`) — ver "Autenticação". Só os endpoints por enquanto; ainda não existe frontend (`admin.html`/`admin.js`) consumindo eles — é a Fase 2.
...
5. **Decisão de arquitetura (alinhada com o gestor, 2026-08-03): ClickUp permanece como backend.** Em vez de trocar de backend pra escalar, o caminho escolhido é construir um **painel de admin** por cima do que já existe, em fases.
   - ✅ **Fase 1 (2026-08-06):** endpoints de backend no Worker — `GET /admin/users` (já existia), mais `GET /admin/tasks` (todos os chamados, com filtros) e `GET /admin/metrics` (agregados de SLA/volume/tempo de atendimento). Ver "Autenticação". Testados em `tests/push-worker.test.js`; ainda **não implantados** (deploy do Worker é manual, colar no dashboard da Cloudflare) nem publicados (`git push`) — aguardando o usuário revisar e sinalizar, conforme a regra de commit deste projeto.
   - ⏳ **Fase 2 (não iniciada):** frontend próprio (`admin.html`/`admin.js`, mesma filosofia zero-dependência) consumindo esses endpoints — tabela de chamados com os filtros, cards de métricas.
   - ⏳ **Fase 3 (não iniciada):** exportação CSV, busca/paginação na tabela, gráfico de SLA (SVG/canvas nativo, sem lib), possíveis ações rápidas (reatribuir operador, etc.).

...
- **Identidade sempre resolvida no servidor:** `handleGetMyTasks`, `handleCreateTask`, `handleGetTask` e `handleSubscribe` nunca confiam em nenhum índice/nome que o cliente mande — resolvem a partir do `name` gravado na sessão. `handleCreateTask` inclusive **sobrescreve** qualquer valor de SOLICITANTE que vier no corpo da requisição.
...
3. **Nota de atendimento no chamado encerrado** (sugerida, não implementada) — 1 a 5 estrelas ou 👍/👎 quando o usuário abre um chamado recém-encerrado; precisa de campo novo na ClickUp + endpoint novo no Worker.
4. Possíveis melhorias futuras: histórico de notificações, métricas de SLA por operador, refinamento do fluxo offline.
